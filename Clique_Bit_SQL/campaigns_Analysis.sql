		...
		with cte as (
				...
				sum(
					case when ( 
                        ...
                        (product_id >=  cast(substring_index(products , '-' , 1) as UNSIGNED ))  and
                        (product_id <=  cast(substring_index(products , '-' , -1) as UNSIGNED ))
                        
                        ) then 1
                        else 0
                       end 
                ) as `campaign_products_added_to_cart`
				from temp
...
select 
campaign_name,
case when impression then 'Received' else 'Not Received' end as Impression,
...
-- ================
select 
	case when click then 'Yes' else 'No' end as `clicked`,
...
from cte1 join cte2
on cte1.dummy_col = cte2.dummy_col;
